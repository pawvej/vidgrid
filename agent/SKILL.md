...
```bash
npx vidgrid clip.mp4                 # → grid.png (+ grid-transcript.txt), prints credits left
npx vidgrid talk.mp4 --grid 4x4 --out talk
npx vidgrid screen.mov --no-transcribe
```

`npx vidgrid <file>` uploads the file to the render API and writes the numbered
...
have ffmpeg; use `npx vidgrid` / the API when you want server-side rendering.
