...
- **`render_video(file_path, grid?, transcribe?)`** — upload a local video file
  (common video extension, max 5 min, max 200 MB); returns the grid PNG(s) +
  transcript. Costs 1 credit; failed renders aren't charged. Does **not** fetch
  URLs — download a link to a file first.
